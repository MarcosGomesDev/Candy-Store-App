...
const ResultSearch = ({route}) => {
    ...
    const {data, isLoading, isError} = useQuery(['search-list'], () => 
        api.searchProducts(name, local.latitude, local.longitude, range, rating), {
        onSuccess: (data) => console.log('deu bom'),
        onError: (e) => console.log(e.response.data),
        retry: 3
    })
    
    const {mutate} = useMutation(() => api.searchProducts(name, local.latitude, local.longitude, range, rating), {
        onSuccess: () => {
